...
Shader "Custom/BlueToYellowShader"
{
	/// <summary>
	/// Shader with 10 bands ranging from Blue (close to observer) to yellow (far from ovserver)
	/// How to use: See HoloAcademy 230 for shader basics
	/// Each shader is needs to be associated with its own material
	/// Then use any input method (clicker, keyboard, voicecommand...) to apply material to
	/// SpatialMappingManager component.
	/// </summary>
	Properties
	{
		_CameraPos("_CameraPos", Vector) = (0,0,0)	// Camera position from main camera
		_Bandwidth("_Bandwidth", float) = 0.25		// the width of each band; Used to allow user to control bandwidth
		_FirstBand("_FirstBand", float) = 1.5		// width of first band; set separately to set "starting distance" of the other bands separately
	}
		SubShader
	{
		...
		PASS
	{
		...
		fixed4 frag(v2f i) : SV_Target
		{
			...
			// 1: first band is handled separately (see above)
			if (distance <= _FirstBand) {
				...
				ret.g = 255 / 255.0f;
				ret.b = 0 / 255.0f;
			} // apply color of last band to anything that is further away than the last band
			else {
				ret.r = 255 / 255.0f;
				ret.g = 255 / 255.0f;
				ret.b = 0 / 255.0f;
